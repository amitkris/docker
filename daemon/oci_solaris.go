...
func setNetworkInterface(s *specs.Spec, c *container.Container) error {
	var anets []specs.Anet
	if c.NetworkSettings != nil && !c.Config.NetworkDisabled {
		lowerlink := ""
		if c.NetworkSettings.Bridge != "" {
			lowerlink = c.NetworkSettings.Bridge
		} else {
			lowerlink = "docker0"
		}
		ep := c.NetworkSettings.Networks["bridge"]
		if ep != nil {
			runzanet := specs.Anet{
				Linkname:          "net0",
				Lowerlink:         lowerlink,
				Allowedaddr:       fmt.Sprintf("%s/%d", ep.IPAddress, ep.IPPrefixLen),
				Configallowedaddr: "true",
				Defrouter:         fmt.Sprintf("%s/%d", ep.Gateway, ep.IPPrefixLen),
...
func (daemon *Daemon) createSpec(c *container.Container) (*libcontainerd.Spec, error) {
	...
	if err := setNetworkInterface(&s, c); err != nil {
		return nil, err
	}
